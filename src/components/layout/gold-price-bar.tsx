...
export function GoldPriceBar() {
  const isUp = GOLD_PRICE.changePercent >= 0;
  const time = new Date(GOLD_PRICE.lastUpdatedAt).toLocaleTimeString("fa-IR", {
    hour: "2-digit",
    minute: "2-digit",
  });
    ...
    <div className="w-full bg-gold-900 text-gold-50">
      <div className="container flex h-9 items-center justify-between gap-4 text-xs sm:text-sm">
        <div className="flex items-center gap-2 overflow-hidden">
          ...
          <span className="font-bold gold-shimmer">{toToman(GOLD_PRICE.pricePerGram18k)}</span>
          <span
          ...
          >
            ...
            {Math.abs(GOLD_PRICE.changePercent)}٪
          </span>
        </div>
